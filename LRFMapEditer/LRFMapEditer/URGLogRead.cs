...
namespace SCIP_library
{
    ...
    /// ログデータ読み込み
    /// </summary>
    public class URGLogRead
    {
        private StreamReader fsr;
        private FileStream fst;

        private List<long> seekPos;     // MDコマンド シーク位置

        public double scale = 1.0;
        public long skipNum = 0;
        public long skipCnt = 0;
        public long nowIdx = 0;    // 次取得した場合のインデックス値
        ...
        public double[] getScanData(long idx)
        {
            if (seekPos.Count < idx)
            {
                throw new ApplicationException("getScanData() Index Error");
            }

            nowIdx = idx;
            fst.Seek( seekPos[(int)idx], SeekOrigin.Begin);
            fsr = new StreamReader(fst, Encoding.GetEncoding("Shift_JIS"));
            double[] getDat = getScanData();
        ...
        public int getNumMD()
        {
            ...
            do {
                seekCur = fst.Seek(0, SeekOrigin.Current);
                str = fsr.ReadLine();
                if (null == str) break;

                if( str.Length == 15 )
                {
                    if (str.Substring(0, 2) == "MD")
                    {
                        cntMD++;
                        // シーク位置辞書作成
                        seekPos.Add(seekCur);
        ...
        public double[] getScanData()
        {
	        ...
	        do
	        {
                ...
                if (strCmd.Equals("MD") && str.Length == 15)
                {
