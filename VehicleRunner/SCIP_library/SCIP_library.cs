...
namespace SCIP_library
{
    ...
    public class SCIP_Reader
    {
        ...
        public static void SetScale(double scaleVal)
        {
            scale = scaleVal;
        }
        ...
        public static bool MD(string get_command, ref long time_stamp, ref List<double> distances)
        {
            distances.Clear();
        ...
        public static bool decode_array(string data, int size, ref List<double> decoded_data)
        {
            for (int pos = 0; pos <= data.Length - size; pos += size) {
                decoded_data.Add((double)decode(data, size, pos) * scale);
            }
            return true;
        }
    }
